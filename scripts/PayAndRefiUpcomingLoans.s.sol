...
import { IDebtLockerLike, IERC20Like, IMapleLoanV3Like, IPoolV1Like } from "../contracts/interfaces/Interfaces.sol";
...
contract PayAndRefinanceUpcomingLoans is SimulationBase {
    ...
    function payLoan(address loan) internal {
        address fundsAsset = IMapleLoanV3Like(loan).fundsAsset();

        ( uint256 principal, uint256 interest, uint256 delegateFee, uint256 treasuryFee ) = IMapleLoanV3Like(loan).getNextPaymentBreakdown();
        ...
        address borrower = IMapleLoanV3Like(loan).borrower();

        vm.broadcast(sender);
        IERC20Like(fundsAsset).transfer(borrower, paymentAmount);

        vm.startBroadcast(IMapleLoanV3Like(loan).borrower());

        IERC20Like(fundsAsset).approve(address(loan), paymentAmount);
        IMapleLoanV3Like(loan).makePayment(paymentAmount);
    ...
    function assertNotClaimable(address loan) internal {
        assertEq(IMapleLoanV3Like(loan).claimableFunds(), 0);
    }
    ...
    function refinanceLoan(address poolV1, address loan) internal {
        ...
        IMapleLoanV3Like(loan).proposeNewTerms(refinancer, type(uint256).max, calls);

        vm.broadcast(poolDelegate);
        IDebtLockerLike(debtLocker).acceptNewTerms(refinancer, type(uint256).max, calls, 0);

        assertEq(IMapleLoanV3Like(loan).gracePeriod(),         0 seconds);
        assertEq(IMapleLoanV3Like(loan).lateInterestPremium(), 0.05e18);
        assertEq(IMapleLoanV3Like(loan).paymentInterval(),     10 days);
        assertEq(IMapleLoanV3Like(loan).paymentsRemaining(),   1);
        assertEq(IMapleLoanV3Like(loan).nextPaymentDueDate(),  block.timestamp + 10 days);
    }
