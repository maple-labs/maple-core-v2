...
import { IDebtLockerLike, IMapleLoanV3Like, IMapleProxyFactory } from "../contracts/interfaces/Interfaces.sol";
...
contract UpgradeDebtLockersTo400 is SimulationBase {
    ...
    function upgradeDebtLockersTo400(address[] memory loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            IDebtLockerLike debtLocker = IDebtLockerLike(IMapleLoanV3Like(loans[i]).lender());

            if (IMapleProxyFactory(debtLockerFactory).versionOf(debtLocker.implementation()) == 400) continue;

            if (IMapleProxyFactory(debtLockerFactory).versionOf(debtLocker.implementation()) == 200) {
                vm.broadcast(debtLocker.poolDelegate());
                debtLocker.upgrade(300, abi.encode(migrationHelperProxy));
            }

            if (IMapleProxyFactory(debtLockerFactory).versionOf(debtLocker.implementation()) == 300) {
                vm.broadcast(debtLocker.poolDelegate());
