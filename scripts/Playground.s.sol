...
import { IERC20Like, IMapleGlobals, IPoolV1Like, IPool } from "../contracts/interfaces/Interfaces.sol";

import { MigrationHelper, SimulationBase } from "../simulations/mainnet/SimulationBase.sol";
...
contract Playground is SimulationBase {
    ...
    function run() external {
        ...
        // console.log("pool.totalSupply()", IPool(pool).totalSupply());
        // console.log("pool.totalAssets()", IPool(pool).totalAssets());

        // for (uint256 i; i < mavenUsdcLps.length; ++i) {
        //     console.log("");
        //     console.log("lpBal ", IPool(pool).balanceOf(mavenUsdcLps[i]));
        //     console.log("lpVal ", IPool(pool).convertToAssets(IPoolV1Like(pool).balanceOf(mavenUsdcLps[i])));
        // }

        // orthogonalLoans.push(0x061fE80139a79DD67eE040c44f19eEe1Fcd04BCC);
        ...
        // IMapleGlobals(mapleGlobals).activatePoolManager(poolManager);
