...
import { IMapleLoanV3Like, IMapleProxyFactory } from "../contracts/interfaces/Interfaces.sol";
...
contract UpgradeLoansTo301 is SimulationBase {
    ...
    function upgradeLoansTo301(address[] storage loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            ...
            if (IMapleProxyFactory(loanFactory).versionOf(loan.implementation()) == 301) continue;

            if (IMapleProxyFactory(loanFactory).versionOf(loan.implementation()) == 200) {
            ...
            if (IMapleProxyFactory(loanFactory).versionOf(loan.implementation()) == 300) {
