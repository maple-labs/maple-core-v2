...
contract QueryPoolV1Positions is ValidationBase {
    ...
    function query(address poolV1, address[] storage lps, string memory name) internal view {
        console.log("");
...
contract PayAndClaimUpcomingLoans is ValidationBase {
    ...
    function validate(address[] storage loans) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            assertTrue(IMapleLoanV3Like(loans[i]).nextPaymentDueDate() > END_MIGRATION);
        }
    }
...
contract RemoveMaturedLoans is ValidationBase {
    ...
    function validate(address[] storage loans) internal view {
        for (uint256 i; i < loans.length; ++i) {
            if (IMapleLoanV3Like(loans[i]).nextPaymentDueDate() != 0) continue;
...
contract DeployProtocol is ValidationBase {
    ...
    function run() external validationConfig {
        ...
        assertEq(IMapleLoanFeeManager(feeManager).globals(), mapleGlobalsV2Proxy);
        ...
        assertEq(IPoolDeployer(poolDeployer).globals(), mapleGlobalsV2Proxy);
        ...
        assertEq(IMapleProxyFactory(liquidatorFactory).versionOf(liquidatorImplementation), 200);
        assertEq(IMapleProxyFactory(liquidatorFactory).migratorForPath(200, 200), liquidatorInitializer);

        assertEq(IMapleProxyFactory(liquidatorFactory).defaultVersion(), 200);

        // Step 16: Configure LoanManagerFactory
        IMapleProxyFactory loanManagerFactory_ = IMapleProxyFactory(loanManagerFactory);
        ...
        assertEq(IMapleProxyFactory(poolManagerFactory).versionOf(poolManagerImplementation), 100);
        assertEq(IMapleProxyFactory(poolManagerFactory).migratorForPath(100, 100), poolManagerInitializer);

        assertEq(IMapleProxyFactory(poolManagerFactory).defaultVersion(), 100);

        // Step 18: Configure WithdrawalManagerFactory
        assertEq(IMapleProxyFactory(withdrawalManagerFactory).versionOf(withdrawalManagerImplementation), 100);
        assertEq(IMapleProxyFactory(withdrawalManagerFactory).migratorForPath(100, 100), withdrawalManagerInitializer);

        assertEq(IMapleProxyFactory(withdrawalManagerFactory).defaultVersion(), 100);
        ...
        assertEq(IHasGLobalsLike(accountingChecker).globals(), mapleGlobalsV2Proxy);

        assertEq(IMigrationHelper(migrationHelperProxy).admin(),          deployer);
        assertEq(IMigrationHelper(migrationHelperProxy).implementation(), migrationHelperImplementation);

        // Step 21: Configure MigrationHelper
        assertEq(IMigrationHelper(migrationHelperProxy).pendingAdmin(), migrationMultisig);
        assertEq(IMigrationHelper(migrationHelperProxy).globalsV2(),    mapleGlobalsV2Proxy);
...
contract SetTemporaryGovernor is ValidationBase {
    ...
    function run() external validationConfig {
        assertEq(IMapleGlobals(mapleGlobalsV2Proxy).pendingGovernor(), address(0),   "pendingGovernor != zero");
        assertEq(IMapleGlobals(mapleGlobalsV2Proxy).governor(),        tempGovernor, "governor != tempGovernor");
    }
...
contract SetMigrationMultisig is ValidationBase {
    ...
    function run() external validationConfig {
        assertEq(IMigrationHelper(migrationHelperProxy).pendingAdmin(), address(0),        "pendingAdmin != zero");
        assertEq(IMigrationHelper(migrationHelperProxy).admin(),        migrationMultisig, "admin != migrationMultisig");
    }
...
contract RegisterDebtLockers is ValidationBase {
    ...
    function run() external validationConfig {
        assertEq(IMapleProxyFactory(debtLockerFactory).versionOf(debtLockerV400Implementation), 400,         "DebtLocker v400 implementation not set");
        assertEq(IMapleProxyFactory(debtLockerFactory).migratorForPath(400, 400), debtLockerV300Initializer, "DebtLocker v400 initializer not set");

        assertEq(IMapleProxyFactory(debtLockerFactory).migratorForPath(300, 400), debtLockerV400Migrator, "DebtLocker v300 to v400 migrator not set");
        assertTrue(IMapleProxyFactory(debtLockerFactory).upgradeEnabledForPath(300, 400),                 "DebtLocker v300 to v400 upgrade disabled");

        assertEq(IMapleProxyFactory(debtLockerFactory).implementationOf(401), debtLockerV401Implementation, "DebtLocker v300 to v400 migrator not set");
        assertEq(IMapleProxyFactory(debtLockerFactory).migratorForPath(400, 401), debtLockerV400Migrator,   "DebtLocker v300 to v400 migrator not set");
        assertTrue(IMapleProxyFactory(debtLockerFactory).upgradeEnabledForPath(400, 401),                   "DebtLocker v300 to v400 upgrade disabled");
    }
...
contract RegisterLoans is ValidationBase {
    ...
    function run() external validationConfig {
        assertEq(IMapleProxyFactory(loanFactory).versionOf(loanV302Implementation), 302,         "Loan v302 implementation not set");
        assertEq(IMapleProxyFactory(loanFactory).migratorForPath(302, 302), loanV300Initializer, "Loan v302 initializer not set");

        assertEq(IMapleProxyFactory(loanFactory).versionOf(loanV400Implementation), 400,         "Loan v400 implementation not set");
        assertEq(IMapleProxyFactory(loanFactory).migratorForPath(400, 400), loanV400Initializer, "Loan v400 initializer not set");

        assertEq(IMapleProxyFactory(loanFactory).migratorForPath(301, 302), address(0), "Loan v301 to v302 migrator is not zero");
        assertTrue(IMapleProxyFactory(loanFactory).upgradeEnabledForPath(301, 302),     "Loan v301 to v302 upgrade disabled");

        assertEq(IMapleProxyFactory(loanFactory).migratorForPath(302, 400), loanV400Migrator, "Loan v302 to v400 migrator not set");
        assertTrue(IMapleProxyFactory(loanFactory).upgradeEnabledForPath(302, 400),           "Loan v302 to v400 upgrade disabled");
    }
...
contract UpgradeDebtLockersToV401 is ValidationBase {
    ...
    function validate(address poolV1, address[] storage loans, uint256 allowedDiff) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            ...
            sumPrincipal += IMapleLoanV3Like(loans[i]).principal();
...
contract QueryMigrationLoans is ValidationBase {
    ...
    function logOut(address loan) internal view {
        console.log("");
        console.log("address           ", loan);
        console.log("principalRequested", IMapleLoanV3Like(loan).principalRequested());
        console.log("fundsAsset        ", IMapleLoanV3Like(loan).fundsAsset());
    }
    ...
    function validate(address poolV1, address migrationLoan, uint256 principalRequested) internal {
        ...
        assertEq(IMapleLoanV3Like(migrationLoan).principalRequested(),   principalRequested);
    }
...
abstract contract FundMigrationLoans is ValidationBase {
    ...
    function validate(address poolV1, address migrationLoan, uint256 funds) internal {
        ...
        IERC20Like asset = IERC20Like(IMapleLoanV3Like(migrationLoan).fundsAsset());

        assertEq(IMapleLoanV3Like(migrationLoan).drawableFunds(), funds);
...
contract QueryMigrationDebtLockers is ValidationBase {
    ...
    function query(address migrationLoan) internal view {
        if (migrationLoan == address(0)) return;

        console.log("loan      ", migrationLoan);
        console.log("debtLocker", IMapleLoanV3Like(migrationLoan).lender());
    }
...
contract UpgradeMigrationDebtLockers is ValidationBase {
    ...
    function validate(address migrationLoan) internal {
        ...
        address debtLocker = IMapleLoanV3Like(migrationLoan).lender();
...
contract QueryPoolV2Info is ValidationBase {
    ...
    function logPoolV2Info(address poolV2) internal view {
        IPoolManager poolManager = IPoolManager(IPool(poolV2).manager());

        console.log("Name", IPool(poolV2).name());
...
contract ActivatePools is ValidationBase {
    ...
    function validate(address poolManager) internal {
        assertTrue(IPoolManager(poolManager).active(), "pool not active");
    }
...
contract GlobalsPoolManagerActivated is ValidationBase {
    ...
    function validate(address poolManager_) internal {
        IMapleGlobals globals_ = IMapleGlobals(mapleGlobalsV2Proxy);

        ( address ownedPoolManager, ) = globals_.poolDelegates(IPoolManager(poolManager_).poolDelegate());
...
contract Airdrop is ValidationBase {
    ...
    function validate(address poolV1, address poolV2, address[] storage lps) internal {
        ...
        for (uint256 i; i < lps.length; ++i) {
            ...
            uint256 v2Position = IPool(poolV2).balanceOf(lps[i]);

            if (lps[i] == 0xB2acd0214F87d217A2eF148aA4a5ABA71d3F7956) {
                v2Position = IPool(poolV2).balanceOf(0x666B8EbFbF4D5f0CE56962a25635CfF563F13161);
        ...
        assertEq(IPool(poolV2).balanceOf(migrationHelperProxy), 0);
    }
...
contract SetPendingLenders is ValidationBase {
    ...
    function validate(address[] storage loans, address tlm) internal {
        for (uint256 i; i < loans.length; ++i) {
            assertEq(IMapleLoanV3Like(loans[i]).pendingLender(), tlm, "pending lender != tlm");
            // TODO: Assert lender
        }
    }
...
contract AcceptPendingLenders is ValidationBase {
    ...
    function validate(address[] storage loans, address tlm) internal {
        for (uint256 i; i < loans.length; ++i) {
            assertEq(IMapleLoanV3Like(loans[i]).lender(),        tlm,        "lender != tlm");
            assertEq(IMapleLoanV3Like(loans[i]).pendingLender(), address(0), "pending lender != 0");
        }
    }
...
contract LoanFactoryNewGlobalsSet is ValidationBase {
    ...
    function validate(address loanFactory_) internal {
        assertEq(IMapleProxyFactory(loanFactory_).mapleGlobals(), mapleGlobalsV2Proxy);
    }
...
contract QueryMigrationLoanFees is ValidationBase {
    ...
    function validate(address migrationLoan) internal view {
        if (migrationLoan == address(0)) return;

        ( , , uint256[2] memory fees ) = IMapleLoan(migrationLoan).getNextPaymentDetailedBreakdown();
...
contract CloseMigrationLoans is ValidationBase {
    ...
    function validate(address poolV1, address migrationLoan) internal {
        assertEq(IMapleLoanV3Like(migrationLoan).nextPaymentDueDate(), 0);
        assertEq(IMapleLoanV3Like(migrationLoan).paymentsRemaining(),  0);

        IERC20Like fundsAsset = IERC20Like(IMapleLoanV3Like(migrationLoan).fundsAsset());

        assertEq(fundsAsset.balanceOf(migrationLoan), 0);
        assertEq(fundsAsset.balanceOf(poolV1),        IMapleLoanV3Like(migrationLoan).principalRequested());
    }
...
contract TransferPoolDelegates is ValidationBase {
    ...
    function validate(address poolManager, address finalPoolDelegate) internal {
        assertEq(IPoolManager(poolManager).poolDelegate(), finalPoolDelegate);
    }
...
contract IncreaseLiquidityCaps is ValidationBase {
    ...
    function validate(address poolManager, uint256 liquidityCap) internal {
        assertEq(IPoolManager(poolManager).liquidityCap(), liquidityCap);
    }
