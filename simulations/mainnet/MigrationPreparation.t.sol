...
contract MigrationPreparationTest is LifecycleBase {
    ...
    function payLoan(address loan) internal {
        address asset    = IMapleLoanV3Like(loan).fundsAsset();
        ...
        (
            ...
            uint256 delegateFee,
            uint256 treasuryFee
        ) = IMapleLoanV3Like(loan).getNextPaymentBreakdown();
        ...
        IMapleLoanV3Like(loan).makePayment(payment);
        vm.stopPrank();
    }
    ...
    function refinanceLoan(address poolV1, address loan) internal {
        ...
        assertEq(IMapleLoanV3Like(loan).paymentInterval(),     30 days);
        assertEq(IMapleLoanV3Like(loan).paymentsRemaining(),   1);
        assertEq(IMapleLoanV3Like(loan).nextPaymentDueDate(),  block.timestamp + 30 days);
    }
    ...
    function propUpWithCash(address poolV2, uint256 amount) internal {
        erc20_mint(IPool(poolV2).asset(), poolV2, amount);
    }
