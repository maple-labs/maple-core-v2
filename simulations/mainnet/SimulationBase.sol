...
import { AccountingChecker }                         from "../../modules/migration-helpers/contracts/AccountingChecker.sol";
...
contract SimulationBase is ProtocolActions, AddressRegistry {
    ...
    function upgradeLoansToV301(address[] storage loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            ...
            uint256 currentVersion = IMapleProxyFactory(loanFactory).versionOf(loan.implementation());
    ...
    function _deployProtocol() internal {
        ...
        IMapleProxyFactory(liquidatorFactory).registerImplementation(200, liquidatorImplementation, liquidatorInitializer);  // Block 16126993
        IMapleProxyFactory(liquidatorFactory).setDefaultVersion(200);                                                        // Block 16126994
        ...
        IMapleProxyFactory(loanManagerFactory).registerImplementation(100, transitionLoanManagerImplementation, loanManagerInitializer);  // Block 16126999
        IMapleProxyFactory(loanManagerFactory).registerImplementation(200, loanManagerImplementation,           loanManagerInitializer);  // Block 16127000
        IMapleProxyFactory(loanManagerFactory).enableUpgradePath(100, 200, address(0));                                                   // Block 16127001
        IMapleProxyFactory(loanManagerFactory).setDefaultVersion(100);                                                                    // Block 16127002
        ...
        IMapleProxyFactory(poolManagerFactory).registerImplementation(100, poolManagerImplementation, poolManagerInitializer);  // Block 16127006
        IMapleProxyFactory(poolManagerFactory).setDefaultVersion(100);                                                          // Block 16127007
        ...
        IMapleProxyFactory(withdrawalManagerFactory).registerImplementation(100, withdrawalManagerImplementation, withdrawalManagerInitializer);  // Block 16127011
        IMapleProxyFactory(withdrawalManagerFactory).setDefaultVersion(100);                                                                      // Block 16127012
        ...
        IMigrationHelper(migrationHelperProxy).setPendingAdmin(migrationMultisig);  // Block 16127024
        IMigrationHelper(migrationHelperProxy).setGlobals(mapleGlobalsV2Proxy);     // Block 16127025
        ...
        for (uint256 i; i < mavenPermissionedLoans.length; ++i) {
            mapleGlobalsV2.setValidBorrower(IMapleLoanV3Like(mavenPermissionedLoans[i]).borrower(), true);
        }

        for (uint256 i; i < mavenUsdcLoans.length; ++i) {
            mapleGlobalsV2.setValidBorrower(IMapleLoanV3Like(mavenUsdcLoans[i]).borrower(), true);
        }

        for (uint256 i; i < mavenWethLoans.length; ++i) {
            mapleGlobalsV2.setValidBorrower(IMapleLoanV3Like(mavenWethLoans[i]).borrower(), true);
        }

        for (uint256 i; i < orthogonalLoans.length; ++i) {
            mapleGlobalsV2.setValidBorrower(IMapleLoanV3Like(orthogonalLoans[i]).borrower(), true);
        }

        for (uint256 i; i < icebreakerLoans.length; ++i) {
            mapleGlobalsV2.setValidBorrower(IMapleLoanV3Like(icebreakerLoans[i]).borrower(), true);
        }
    ...
    function tempGovernorAcceptsV2Governorship() internal {
        ...
        IMapleGlobals(mapleGlobalsV2Proxy).acceptGovernor();  // Block 16136610
    }
    ...
    function migrationMultisigAcceptsMigrationAdministratorship() internal {
        ...
        IMigrationHelper(migrationHelperProxy).acceptOwner();  // Block 16136630
    }
    ...
    function setupExistingFactories() internal {
        ...
        IMapleProxyFactory debtLockerFactory_ = IMapleProxyFactory(debtLockerFactory);
        ...
        IMapleProxyFactory loanFactory_ = IMapleProxyFactory(loanFactory);
    ...
    function setUpDebtLockerFactoryFor401() internal {
        ...
        IMapleProxyFactory(debtLockerFactory).registerImplementation(401, debtLockerV401Implementation, address(0));  // Block 16155441
        IMapleProxyFactory(debtLockerFactory).enableUpgradePath(400, 401, debtLockerV400Migrator);                    // Block 16155441
    ...
    function upgradeDebtLockersToV400(address[] storage loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            ...
            if (IMapleProxyFactory(debtLockerFactory).versionOf(debtLocker.implementation()) == 400) return;
    ...
    function upgradeDebtLockersToV401(address[] storage loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            ...
            if (IMapleProxyFactory(debtLockerFactory).versionOf(debtLocker.implementation()) == 401) return;
    ...
    function claimLoans(address poolV1, address[] storage loans) internal {
        ...
        for (uint256 i; i < loans.length;) {
            ...
            IPoolV1Like(poolV1).claim(address(loan), IMapleProxied(loan.lender()).factory());
    ...
    function upgradeLoansToV302(address[] storage loans) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            ...
            IMapleLoanV3Like(loan).upgrade(302, new bytes(0));
    ...
    function createMigrationLoan(address poolV1, address[] storage loans, uint256 liquidity) internal returns (address migrationLoan) {
        ...
        migrationLoan     = IMapleProxyFactory(loanFactory).createInstance(args, salt);
    ...
    function fundMigrationLoan(address poolV1, address migrationLoan) internal {
        ...
        uint256 principalRequested = IMapleLoanV3Like(migrationLoan).principalRequested();
    ...
    function upgradeDebtLockerToV400(address loan) internal {
        IDebtLockerLike debtLocker = IDebtLockerLike(IMapleLoanV3Like(loan).lender());
    ...
    function deployPoolV2(address temporaryPD, address poolV1) internal returns (address poolManager) {
        ...
        address[3] memory initializers = [
            IMapleProxyFactory(poolManagerFactory).migratorForPath(100, 100),
            IMapleProxyFactory(loanManagerFactory).migratorForPath(100, 100),
            IMapleProxyFactory(withdrawalManagerFactory).migratorForPath(100, 100)
        ];
    ...
    function deployAllPoolV2s() internal {
        ...
        mavenPermissionedPoolV2 = IPoolManager(mavenPermissionedPoolManager).pool();
        mavenUsdcPoolV2         = IPoolManager(mavenUsdcPoolManager).pool();
        mavenWethPoolV2         = IPoolManager(mavenWethPoolManager).pool();
        orthogonalPoolV2        = IPoolManager(orthogonalPoolManager).pool();
        icebreakerPoolV2        = IPoolManager(icebreakerPoolManager).pool();
    }

    // Liquidity Migration Procedure #19 [Block 16162671]
    function setFees() internal {
        IMapleGlobals globalsV2 = IMapleGlobals(mapleGlobalsV2Proxy);
    ...
    function addLoansToLoanManager(address poolManager, address poolV1, address[] storage loans, uint256 allowedDiff) internal {
        ...
        address transitionLoanManager = IPoolManager(poolManager).loanManagerList(0);

        vm.prank(migrationMultisig);
        IMigrationHelper(migrationHelperProxy).addLoansToLoanManager(poolV1, transitionLoanManager, loans, allowedDiff);
    ...
    function activatePoolManager(address poolManager) internal {
        vm.prank(tempGovernor);
        IMapleGlobals(mapleGlobalsV2Proxy).activatePoolManager(poolManager);
    }
    ...
    function allowLendersAndWithdrawalManager(address poolManager, address[] storage lenders) internal {
        ...
        allowLender(poolManager, IPoolManager(poolManager).withdrawalManager());
    }
    ...
    function replaceSherlockAddress(address[] storage lps) internal view returns (address[] memory lps2) {
    ...
    function airdropTokens(address poolV1, address poolManager, address[] storage lps) internal {
        ...
        IMigrationHelper(migrationHelperProxy).airdropTokens(poolV1, poolManager, lps, replaceSherlockAddress(lps), lps.length * 2);
        vm.stopPrank();
    }
    ...
    function setPendingLenders(address poolV1, address poolManager, address[] storage loans, uint256 allowedDiff) internal {
        vm.startPrank(migrationMultisig);
        IMigrationHelper(migrationHelperProxy).setPendingLenders(poolV1, poolManager, loanFactory, loans, allowedDiff);
        vm.stopPrank();
    }
    ...
    function takeOwnershipOfLoans(address poolV1, address poolManager, address[] storage loans, uint256 allowedDiff) internal {
        ...
        IMigrationHelper(migrationHelperProxy)
            .takeOwnershipOfLoans(
                poolV1,
                IPoolManager(poolManager).loanManagerList(0),
                loans,
                allowedDiff
            );
    ...
    function upgradeLoanManager(address transitionLoanManager) internal {
        vm.startPrank(migrationMultisig);
        IMigrationHelper(migrationHelperProxy).upgradeLoanManager(transitionLoanManager, 200);
        vm.stopPrank();
    }

    // Liquidity Migration Procedure #26 [Block 16164645]
    function upgradeAllLoanManagers() internal {
        upgradeLoanManager(IPoolManager(mavenPermissionedPoolManager).loanManagerList(0));
        upgradeLoanManager(IPoolManager(mavenUsdcPoolManager).loanManagerList(0));
        upgradeLoanManager(IPoolManager(mavenWethPoolManager).loanManagerList(0));
        upgradeLoanManager(IPoolManager(orthogonalPoolManager).loanManagerList(0));
        upgradeLoanManager(IPoolManager(icebreakerPoolManager).loanManagerList(0));
    }
    ...
    function upgradeLoansToV400(address[] storage loans) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            address loan = loans[i];
            IMapleLoanV3Like(loan).upgrade(400, abi.encode(feeManager));
    ...
    // Liquidity Migration Procedure #29 [Block 16164991]
    function closeMigrationLoanIfNeeded(address poolManager, address migrationLoan, address[] storage loans) internal {
    ...
    function closeAllMigrationLoans() internal {
        closeMigrationLoanIfNeeded(mavenPermissionedPoolManager, mavenPermissionedMigrationLoan, mavenPermissionedLoans);
        closeMigrationLoanIfNeeded(mavenUsdcPoolManager,         mavenUsdcMigrationLoan,         mavenUsdcLoans);
        closeMigrationLoanIfNeeded(mavenWethPoolManager,         mavenWethMigrationLoan,         mavenWethLoans);
        closeMigrationLoanIfNeeded(orthogonalPoolManager,        orthogonalMigrationLoan,        orthogonalLoans);
        closeMigrationLoanIfNeeded(icebreakerPoolManager,        icebreakerMigrationLoan,        icebreakerLoans);
    }
    ...
    function finalizeFactories() internal {
        vm.startPrank(tempGovernor);
        IMapleProxyFactory(loanFactory).setDefaultVersion(400);
        IMapleProxyFactory(loanManagerFactory).setDefaultVersion(200);
        vm.stopPrank();
    }
    ...
    function enableFinalPoolDelegates() internal {
        IMapleGlobals mapleGlobalsV2 = IMapleGlobals(mapleGlobalsV2Proxy);
    ...
    function tempGovernorTransfersV2Governorship() internal {
        vm.prank(tempGovernor);
        IMapleGlobals(mapleGlobalsV2Proxy).setPendingGovernor(governor);
    }

    // Liquidity Migration Procedures #42
    function governorAcceptsV2Governorship() internal {
        vm.prank(governor);
        IMapleGlobals(mapleGlobalsV2Proxy).acceptGovernor();
    }
    ...
    function performAdditionalGlobalsSettings() internal {
        ...
        IMapleGlobals(mapleGlobalsV2Proxy).setValidPoolAsset(wbtc, true);
        IMapleGlobals(mapleGlobalsV2Proxy).setValidCollateralAsset(usdc, true);

        vm.stopPrank();
    }

    function setMinCoverAmount(address poolManager, uint256 coverAmount) internal {
        // Configure the min cover amount in globals
        vm.prank(tempGovernor);
        IMapleGlobals(mapleGlobalsV2Proxy).setMinCoverAmount(poolManager, coverAmount);
    }

    function setAllMinCoverAmounts() internal {
        setMinCoverAmount(mavenPermissionedPoolManager, 1_750_000e6);
        setMinCoverAmount(mavenUsdcPoolManager,         1_000_000e6);
        setMinCoverAmount(mavenWethPoolManager,         750e18);
        setMinCoverAmount(orthogonalPoolManager,        2_500_000e6);
        setMinCoverAmount(icebreakerPoolManager,        500_000e6);
    }
    ...
    function checkSumOfLoanPrincipal(address poolV1, address[] storage loans, uint256 allowedDiff) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            totalPrincipal += IMapleLoanV3Like(loans[i]).principal();
        }
    ...
    function assertLoansBelongToPool(address poolV1, address[] storage loans) internal {
        for (uint256 i; i < loans.length; ++i) {
            assertEq(
                IDebtLockerLike(IMapleLoanV3Like(loans[i]).lender()).pool(),
                poolV1
            );
        }
    }
    ...
    function assertPoolAccounting(address poolManager, address[] storage loans, uint256 hoursToWarp) internal {
        ...
        for (uint256 i; i <= hoursToWarp; ++i) {
            ...
            ) = IAccountingChecker(accountingChecker).checkPoolAccounting(poolManager, loans, loansAddedTimestamp, lastUpdatedTimestamp);
    ...
    function assertPrincipalOut(address loanManager, address[] storage loans) internal {
        ...
        assertEq(ILoanManager(loanManager).principalOut(), totalPrincipal);
    }

    function assertAllPrincipalOuts() internal {
        assertPrincipalOut(IPoolManager(mavenPermissionedPoolManager).loanManagerList(0), mavenPermissionedLoans);
        assertPrincipalOut(IPoolManager(mavenUsdcPoolManager).loanManagerList(0),         mavenUsdcLoans);
        assertPrincipalOut(IPoolManager(mavenWethPoolManager).loanManagerList(0),         mavenWethLoans);
        assertPrincipalOut(IPoolManager(orthogonalPoolManager).loanManagerList(0),        orthogonalLoans);
        assertPrincipalOut(IPoolManager(icebreakerPoolManager).loanManagerList(0),        icebreakerLoans);
    }

    function assertTotalSupply(address poolManager, address poolV1) internal {
        assertEq(
            IPoolV1Like(
                IPoolManager(poolManager).pool()
            ).totalSupply(),
            getPoolV1TotalValue(poolV1)
        );
    }
    ...
    function assertVersion(uint256 version , address instance) internal {
        assertEq(
            IMapleProxied(instance).implementation(),
            IMapleProxyFactory(IMapleProxied(instance).factory()).implementationOf(version),
            "Mismatched Versions"
        );
    }
    ...
    function compareLpPositions(address poolV1, address poolV2, address[] storage lps) internal {
        ...
        uint256 poolV2TotalSupply = IPool(poolV2).totalSupply();
        uint256 sumPosition       = getSumPosition(poolV1, lps);

        for (uint256 i; i < lps.length; ++i) {
            uint256 v1Position = getV1Position(poolV1, lps[i]);
            uint256 v2Position = IPool(poolV2).balanceOf(lps[i] == 0xB2acd0214F87d217A2eF148aA4a5ABA71d3F7956 ? 0x666B8EbFbF4D5f0CE56962a25635CfF563F13161 : lps[i]);
    ...
    function compareAllLpPositions() internal {
        compareLpPositions(mavenWethPoolV1,         IPoolManager(mavenWethPoolManager).pool(),         mavenWethLps);
        compareLpPositions(mavenUsdcPoolV1,         IPoolManager(mavenUsdcPoolManager).pool(),         mavenUsdcLps);
        compareLpPositions(mavenPermissionedPoolV1, IPoolManager(mavenPermissionedPoolManager).pool(), mavenPermissionedLps);
        compareLpPositions(orthogonalPoolV1,        IPoolManager(orthogonalPoolManager).pool(),        orthogonalLps);
        compareLpPositions(icebreakerPoolV1,        IPoolManager(icebreakerPoolManager).pool(),        icebreakerLps);
    }
    ...
    function getNextLoanAndPaymentDueDate(address[] storage loans) internal view returns (address loan, uint256 nextPaymentDueDate) {
        for (uint256 i; i < loans.length; ++i) {
            uint256 dueDate = IMapleLoanV3Like(loans[i]).nextPaymentDueDate();
    ...
    function setGlobalsOfFactory(address factory, address globals) internal {
        IMapleProxyFactory factory_ = IMapleProxyFactory(factory);
    ...
    function enableLoanDowngradeFromV301() internal {
        vm.prank(governor);
        IMapleProxyFactory(loanFactory).enableUpgradePath(301, 300, address(0));
    }

    // Rollback Liquidity Migration Procedure #4
    function disableLoanUpgradeFromV300() internal {
        IMapleProxyFactory factory = IMapleProxyFactory(loanFactory);
    ...
    function enableDebtLockerDowngradeFromV400() internal {
        vm.prank(governor);
        IMapleProxyFactory(debtLockerFactory).enableUpgradePath(400, 300, address(0));
    }

    // Rollback Liquidity Migration Procedure #9.1
    function disableDebtLockerUpgradeFromV300() internal {
        vm.prank(governor);
        IMapleProxyFactory(debtLockerFactory).disableUpgradePath(300, 400);
    }
    ...
    function downgradeDebtLockersFromV400(address[] storage loans) internal {
        ...
        for (uint256 i; i < loans.length; ++i) {
            address debtLocker = IMapleLoanV3Like(loans[i]).lender();
    ...
    function enableDebtLockerDowngradeFromV401() internal {
        vm.prank(governor);
        IMapleProxyFactory(debtLockerFactory).enableUpgradePath(401, 400, address(0));
    }

    // Rollback Liquidity Migration Procedure #9.2
    function disableDebtLockerUpgradeFromV400() internal {
        vm.prank(governor);
        IMapleProxyFactory(debtLockerFactory).disableUpgradePath(400, 401);
    }
    ...
    function enableLoanDowngradeFromV302() internal {
        vm.prank(governor);
        IMapleProxyFactory(loanFactory).enableUpgradePath(302, 301, address(0));
    }

    // Rollback Liquidity Migration Procedure #11
    function disableLoanUpgradesFromV301() internal {
        vm.startPrank(governor);
        IMapleProxyFactory(loanFactory).disableUpgradePath(301, 302);
        IMapleProxyFactory(loanFactory).disableUpgradePath(302, 400);
        vm.stopPrank();
    }
    ...
    function paybackMigrationLoanToPoolV1(address poolV1, address migrationLoan, address[] storage loans) internal {
        ...
        vm.prank(migrationMultisig);
        IMapleLoanV3Like(migrationLoan).closeLoan(0);
    ...
    function downgradeDebtLockerFromV400(address loan) internal {
        ...
        address debtLocker = IMapleLoanV3Like(loan).lender();
    ...
    function downgradeMigrationLoansFromV302(address loan) internal {
        ...
        uint256 currentVersion = IMapleProxyFactory(loanFactory).versionOf(IMapleLoanV3Like(loan).implementation());

        if (currentVersion == 200 || currentVersion == 301) return;

        vm.prank(globalAdmin);
        IMapleLoanV3Like(loan).upgrade(301, new bytes(0));
    ...
    function unsetPendingLenders(address[] storage loans) internal {
        vm.prank(migrationMultisig);
        IMigrationHelper(migrationHelperProxy).rollback_setPendingLenders(loans);
    }
    ...
    function revertOwnershipOfLoans(address poolManager, address[] storage loans_) internal {
        ...
        vm.prank(migrationMultisig);
        IMigrationHelper(migrationHelperProxy).rollback_takeOwnershipOfLoans(loanManager, loans_);
    }
    ...
    function enableLoanManagerDowngradeFromV200() internal {
        vm.prank(tempGovernor);
        IMapleProxyFactory(loanManagerFactory).enableUpgradePath(200, 100, address(0));
    }
    ...
    function downgradeLoanManagerFromV200(address poolManager) internal {
        ...
        vm.prank(tempGovernor);
        IMapleProxied(loanManager).upgrade(100, new bytes(0));
    ...
    function enableLoanDowngradeFromV400() internal {
        vm.prank(governor);
        IMapleProxyFactory(loanFactory).enableUpgradePath(400, 302, address(0));
    }
